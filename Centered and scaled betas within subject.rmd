...
composite_graph = graph1 + graph2 + graph4
composite_graph
```
Revised plots 07172023
```{r}
graph1 <- ggplot(df_unnormalized_s_pooled_means, aes(x = value, fill = lane)) +
  geom_histogram(position = 'identity', alpha = 0.8, binwidth = 0.075, show.legend = FALSE) +
  theme_bw() + 
  facet_grid( lane ~ . ) +
...
# same as last figure, but with the legend there so we can crop this
graph4 <- ggplot(df_normalized_with_combat_s_pooled_means, aes(x = value, fill = lane)) +
  geom_histogram(position = 'identity', alpha = 0.8, binwidth = 0.075) +
  theme_bw() +
  facet_grid( lane ~ . ) +
  theme(text = element_text(size = 5))  +
  ggtitle("SeSAMe + ComBat")
...
pdf(file = paste0(pdf_export_directory, "distributions_of_centered_and_scaled_betas.pdf.pdf"),   # The directory you want to save the file in
    width = (130/25.4), # The width of the plot in inches (converted from mm)
...
<!-- # save_pdf -->
<!-- pdf("/home/sagemaker-user/dnamstability/dnamstability-abutler/pdf_plots/Distributions_of_centered_and_scaled_betas.pdf",width = 5, height = 3,pointsize=10) -->
<!-- composite_graph -->
<!-- dev.off() -->
<!-- ``` -->
...
```{r}
library(monochromeR)
greys = generate_palette("grey", modification = "go_darker", 
...
### Plotting
pdf_export_directory = '~/dnamstability/dnamstability-abutler/pdf_plots/' #where I'm saving figures
pdf(file = paste0(pdf_export_directory, "distributions_of_centered_and_scaled_betas_by_array_140.pdf"),   # The directory you want to save the file in
    width = (130/25.4), # The width of the plot in inches (converted from mm)
...
# Average within arrays, so that we don't mix array and chamber number variation 
...
# Note which samples are repeated more than once on each array to ensure scaling is all within-array
SampleAnnotation$withinArrayScaling = FALSE
for (i in 1:length(SampleAnnotation$withinArrayScaling)){
  if (SampleAnnotation$subjectLetter[i]=="A" & SampleAnnotation$array[i]=="205832330169"){
    SampleAnnotation$withinArrayScaling[i] = TRUE
  }
  if (SampleAnnotation$subjectLetter[i]=="B" & SampleAnnotation$array[i]=="205837540011"){
    SampleAnnotation$withinArrayScaling[i] = TRUE
  }
  if (SampleAnnotation$subjectLetter[i]=="C" & SampleAnnotation$array[i]=="205837540013"){
    SampleAnnotation$withinArrayScaling[i] = TRUE
  }
  if (SampleAnnotation$subjectLetter[i]=="D" & SampleAnnotation$array[i]=="205837540026"){
    SampleAnnotation$withinArrayScaling[i] = TRUE
  }
}

samples_to_keep = SampleAnnotation %>%
  filter(SampleAnnotation$withinArrayScaling==TRUE)

df_normalized_single_array = df_normalized %>%
  select(samples_to_keep$XChipID)



df_normalized_s = scale_data_by_grouping_variable(data = df_normalized_single_array, 
                                                  annotation_df = samples_to_keep,
                                                  subject_col = "XChipID", 
                                                  grouping_var = "subjectLetter")
...
# pooled samples
df_normalized_s_pooled = df_normalized_s %>%
  dplyr::select(samples_to_keep$XChipID[samples_to_keep$pooled==TRUE])
...
for (i in 1:length(positions)){
  temp = df %>%
    dplyr::select(samples_to_keep$XChipID[samples_to_keep$pooled==TRUE]) %>%
    dplyr::select(any_of(samples_to_keep$XChipID[samples_to_keep$lane==positions[i]])) %>% apply(1,mean,na.rm=TRUE)
  if (i == 1){
    temp2 = data.frame(CpG = rownames(df),
                       temp1 = temp)
  } else {
...
Melt columns and add in ID variables
```{r}
ids_to_bind = t(samples_to_keep)
ids_to_bind = data.frame(ids_to_bind)
colnames(ids_to_bind) = ids_to_bind["XChipID",]

# putting variable info into a single string string (new column name) to preserve while melting
samples_to_keep$BigName = paste(samples_to_keep$XChipID, 
                                 samples_to_keep$subjectLetter,
                                 samples_to_keep$pooled,
                                 samples_to_keep$grouping_variable_bswa,
                                 samples_to_keep$grouping_variable,
                                 samples_to_keep$grouping_variable2,
                                 samples_to_keep$array,
                                 samples_to_keep$lane,
                                 sep = '.')

for (i in 1:length(colnames(df_normalized_s))){
  j = which(samples_to_keep$XChipID==colnames(df_normalized_s)[i])
  colnames(df_normalized_s)[i] = samples_to_keep$BigName[j]
}
df_normalized_s = reshape2::melt(df_normalized_s)
...
graph2<- ggplot(df_normalized_s_pooled_means, aes(x = value, fill = lane)) +
  ...
  facet_grid( lane ~ . ) +
  ggtitle("Distribution of centered and scaled beta values in normalized data",
          "Each subject only in a single array")
graph2

#ggplotgui::ggplot_shiny(df_normalized_s_pooled_means)

graph <- ggplot(df_normalized_s_pooled_means, aes(x = value)) +
  geom_histogram(position = 'identity', alpha = 0.8, binwidth = 0.05) +
  facet_grid( lane ~ subject ) +
  theme_bw()
graph
